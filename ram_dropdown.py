...
class SpecsWindow(Gtk.Window):
    def __init__(self):
        ...
        # RAM
        ram_lbl = Gtk.Label()
        ram_lbl.set_markup("<span font_desc='FontAwesome 12'></span>  Memory Usage:")
    ...
    def update_specs(self):
        try:
            uname = subprocess.check_output("uname -srv", shell=True).decode('utf-8').strip()
            self.cpu_val.set_text(uname)
            ...
            self.ram_val.set_text(f"{used} MB / {total} MB  ({free_mem} MB Free)")
            ...
            if len(df) > 1:
                ...
                if len(disk_parts) >= 5:
                    ...
                    self.disk_val.set_text(f"{d_used} / {d_total}  ({d_avail} Available)")
